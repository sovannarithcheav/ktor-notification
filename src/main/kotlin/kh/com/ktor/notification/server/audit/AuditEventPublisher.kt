...
object AuditEventPublisher {
    ...
    private const val EXCHANGE    = "audit.events"
    private const val ROUTING_KEY = "audit"
    ...
    fun publish(event: AuditEventMessage) {
        val conn = connection ?: return
        try {
            val normalized = event.copy(
                activity = event.activity.lowercase(),
                module   = event.module.lowercase(),
            )
            val ch    = conn.createChannel()
            val body  = Json.encodeToString(normalized).toByteArray(Charsets.UTF_8)
